...
const FeatSection = () => {
  const renderFeatureItems = () => {
    return FeatureInfo.map((elem,i) => (
      <div
        key={i}
        className="lg:space-y-4 lg:rounded-2xl  md:px-8 md:w-[28rem] cursor-pointer px-4 py-4 hover:border-white hover:bg-white hover:shadow-lg hover:shadow-black/20 md:h-[10rem]"
      >
        <motion.h1 variants={itemVariants} className="text-lg font-bold">{elem.heading}</motion.h1>
        <motion.ul variants={itemVariants} className="list-inside list-disc space-y-4 pl-4 text-base font-normal text-secondaryBGgray">
          <li>{elem.sub1}</li>
          <li>{elem.sub2}</li>
        </motion.ul>
      </div>
    ...
    <div className="flex min-h-screen w-full flex-wrap items-center justify-between bg-textWhite px-4 pt-20 md:px-8 lg:flex-nowrap lg:px-36 lg:py-36">
      ...
      <motion.div
         variants={containerVariants}
          ...
          whileInView="show"
          viewport={{ once: true, amount: 0.3 }}
        className="space-y-6 py-10 md:px-10">{renderFeatureItems()}</motion.div>
    </div>
  );
};
